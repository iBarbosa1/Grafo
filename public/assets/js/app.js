(function () {
  ...
  var ARQUIVOS_IMAGEM = ["OLT.jpeg", "SWITCH.jpeg", "MIROTIK.png", "MIROTIK.jpeg", "NOBREAK.jpeg", "RTIFICADOR.jpeg", "VOLT.jpeg", "grafo.png"];
  ...
  function caminhoImagem(nomeArquivo) {
    var caminho = String(nomeArquivo || "").trim().replace(/\\/g, "/");
    if (!caminho) return "";
    if (/^(data:|blob:|https?:|\/\/)/i.test(caminho)) return caminho;

    // Projetos antigos podem guardar apenas o nome ou um caminho relativo.
    var arquivo = caminho.split(/[?#]/)[0].split("/").pop();
    var conhecido = ARQUIVOS_IMAGEM.find(function (item) {
      return item.toLowerCase() === arquivo.toLowerCase();
    });
    if (conhecido) return ASSET_BASE + conhecido;
    if (caminho.charAt(0) === "/") return caminho;
    if (caminho.indexOf("/") < 0) return ASSET_BASE + caminho;
    return "/" + caminho.replace(/^(\.\/)+/, "");
  }

  function resolverUrlRelativa(origem) {
    if (!origem) return origem;
    var caminho = caminhoImagem(origem);
    if (/^(data:|blob:|https?:|\/\/)/i.test(caminho)) return caminho;
    try { return new URL(caminho, window.location.origin).href; }
  ...
  function imagemDoIcone(valor) {
    ...
    if (/\.(?:png|jpe?g|webp|gif|svg)(?:[?#].*)?$/i.test(String(valor))) return caminhoImagem(valor);
  ...
  async function prepararSvgSeguroParaPng(svgEl) {
    ...
    for (var i = 0; i < imagensCopia.length; i++) {
      ...
      try {
        ...
        var resposta = await fetch(absoluta, { cache: "force-cache" });
        if (!resposta.ok) throw new Error("HTTP " + resposta.status);
        var arquivoImagem = await resposta.blob();
        if (!arquivoImagem.type.startsWith("image/")) throw new Error("O endereço não retornou uma imagem.");
        var dataUrl = await blobParaDataUrl(arquivoImagem);
        var urlFinal = resolverUrlRelativa(dataUrl);
        imagem.setAttribute("href", urlFinal);
        imagem.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href", urlFinal);
      } catch (erroImagemPng) {
        throw new Error("Não foi possível incluir a imagem " + origem + " no PNG: " + erroImagemPng.message);
      }
    }
